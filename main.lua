...
function file_loaded()
	...
	if youtube_id then
--		if not string.len(youtube_id) < 11 then return end
		youtube_id = string.sub(youtube_id, -11, -1)
	end
	if youtube_id5 then
		vtitle = mp.get_property("media-title")
		youtube_id_pre = io.popen("youtube-dl --get-id \"ytsearch1:"..vtitle.."\"")
		youtube_id = youtube_id_pre:read("*a")
		youtube_id_pre:close()
	end
	getranges()
	if ranges then
		ON = true
